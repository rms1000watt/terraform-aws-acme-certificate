locals {
  ...
  name_cert_only_pem = "${local.common_name}/cert-only.pem"
  ...
  certificate_certificate_only_pem = "${element(compact(concat(acme_certificate.w_sans.*.certificate_pem, acme_certificate.n_sans.*.certificate_pem,list("nil"))),0)}"
  certificate_certificate_pem      = "${local.certificate_certificate_only_pem}${local.certificate_issuer_pem}"
  ...
  out_private_key_pem      = "${local.certificate_private_key_pem}"
  out_certificate_only_pem = "${local.certificate_certificate_only_pem}"
  out_certificate_pem      = "${local.certificate_certificate_pem}"
  out_issuer_pem           = "${local.certificate_issuer_pem}"
}
